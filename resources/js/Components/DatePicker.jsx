...
export default function DatePicker({ searchFieldChanged, queryParams }) {
    ...
    const renderCalendar = () => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            let className =
                "flex font-oxanium items-center justify-center cursor-pointer w-[46px] h-[46px] rounded-full text-dark-3 dark:text-dark-6 hover:bg-primary hover:text-black";

            if (selectedStartDate && dayString === selectedStartDate) {
                className += " bg-yellowTheme font-oxanium text-black rounded-r-none";
            }
            if (selectedEndDate && dayString === selectedEndDate) {
                className += " bg-yellowTheme font-oxanium text-black rounded-l-none";
            }
            if (
                selectedStartDate &&
                selectedEndDate &&
                ...
                className += " bg-yellowTheme font-oxanium text-black rounded-none";
            }
    ...
    const updateInput = () => {
        if (queryParams['dateStart'] && queryParams["dateEnd"]) {
            if (queryParams['dateStart'] == queryParams["dateEnd"]){
                return moment(queryParams['dateStart']).format('DD/MM/YYYY');
            }
            let start = moment(queryParams['dateStart']).format('DD/MM/YYYY')
            let end = moment(queryParams['dateEnd']).format('DD/MM/YYYY')
            return `${start} - ${end}`;
        } else if (queryParams['dateStart']) {
            return moment(queryParams['dateStart']).format('DD/MM/YYYY');
        } else {
            return "";
        }
    ...
    const handleApply = () => {
        searchFieldChanged('dateStart', selectedStartDate)
        if(selectedEndDate){
            searchFieldChanged('dateEnd', selectedEndDate)
        }
        setIsOpen(false);
    };
    ...
    const handleCancel = () => {
        ...
        searchFieldChanged('dateStart','')
        searchFieldChanged('dateEnd','')
    };
    ...
    return (
        <section className="cursor-pointer lg:w-fit bg-white w-full font-oxanium">
                            ...
                            <div className="relative" ref={datepickerRef}>
                                <div className="relative flex items-center">
                                    ...
                                    <input
                                        ...
                                        className="w-full rounded-md font-semibold border-gray-700  border-[3px] placeholder:italic text-black italic focus:border-greenTheme focus:ring-greenTheme placeholder:text-blackTheme border-stroke bg-transparent py-2.5 pl-[50px] pr-8 text-dark-2 outline-none transition focus:border-primary dark:border-dark-3 dark:text-dark-6 dark:focus:border-primary"
                                        value={updateInput()}
                                ...
                                {isOpen && (
                                    <div
                                        ...
                                        className="shadow-datepicker absolute mt-2 rounded-xl border border-stroke bg-white md:scale-100 scale-75 origin-top-left pt-5 dark:border-dark-3 dark:bg-dark-2"
                                    >
                                        <div className="flex items-center justify-between px-5">
                                            <button
                                                id="prevMonth"
                                                className="rounded-md px-2 py-2 text-dark hover:bg-gray-2  dark:hover:bg-dark"
                                                onClick={() =>
                                                    setCurrentDate(
                                                        new Date(
                                        ...
                                        <div className="mt-5 flex justify-end space-x-2.5 border-t border-stroke p-5 dark:border-dark-3">
                                            ...
                                            <button
                                                id="applyButton"
                                                className="rounded-lg bg-primary px-5 py-2.5 text-base font-medium text-yellowTheme bg-blackTheme font-oxanium text-black-110"
                                                onClick={handleApply}
                                            >
                                                Apply
                                            </button>
